...
from .modules import gui as cheater_gui
...
class App:
    ...
    def get_args(self):
        ...
        parser = argparse.ArgumentParser(
            prog="cheater",
            description='cheater v{} - Pentest command launcher'.format(__version__),
            epilog=examples,
    ...
    def start(self, args, cheatsheets):
        cheater_gui.Gui.with_tags = args.no_tags
                ...
                if cmd.cmdline == ">exit":
                    ...
                    if (os.path.exists(config.savevarfile)):
                        with open(config.savevarfile, 'r') as f:
                            cheaterGlobalVars = json.load(f)
                            for k, v in cheaterGlobalVars.items():
                                print(k + "=" + v)
                    break
                    ...
                    for v in varlist:
                        cheaterGlobalVars[v[0]] = v[1]
                    with open(config.savevarfile, "w") as f:
                        f.write(json.dumps(cheaterGlobalVars))
                else:
                    print("Cheater: invalid internal command..")
                    break
    ...
    def prefil_shell_cmd(self, cmd):
        ...
        try:
            ...
            message = "========== OSError ============\n"
            message += "Cheater needs TIOCSTI enable for running\n"
            ...
            message += "More details about this bug here: https://github.com/Orange-Cyberdefense/cheater/issues/77"
            print(message)
